...
class DBIP_Client {
	...
	protected function Do_API_Call($path = "") {
		...
		if (isset($this->lang)) {
			$jdata = @file_get_contents($url, false, stream_context_create([
				"http" => [
		...
		if (!$jdata) {
			throw new DBIP_Client_Exception("unable to fetch URL: {$url}");
		}
		if (!$data = @json_decode($jdata)) {
			throw new DBIP_Client_Exception("cannot decode server response");
		}
		if (isset($data->error)) {
			throw new DBIP_Client_Exception("server reported an error: {$data->error}");
		}
		return $data;
	}
